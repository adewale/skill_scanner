...
- **Image metadata** (`_scan_image_metadata`) -- PNG `tEXt`/`zTXt`/`iTXt` chunks, JPEG `COM`/EXIF segments, GIF comment/application extensions, and WebP `EXIF`/`XMP ` RIFF chunks are decoded with the standard library (`zlib`), then run through the full pattern engine plus an instruction-shaped-text heuristic. Images are no longer skipped wholesale.
